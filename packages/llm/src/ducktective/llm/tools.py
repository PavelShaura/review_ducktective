...
"""Нижняя граница того, сколько от ответа инструмента видит модель.

Действует, пока окно модели неизвестно; при известном окне предел
выводится из него (`result_chars_for`). Результат остаётся в диалоге
до конца цикла, и при 16 000 четыре неурезанных ответа не оставят места
ни на дифф, ни на сам разбор.
"""

RESULTS_PER_WINDOW = 32
"""На сколько результатов инструментов делится окно модели.

Восемь результатов занимают четверть окна; остальное — промпт, дифф и ответ.
"""

CHARS_PER_TOKEN = 3
"""Курс оценки токенов по длине текста.

Взят по нижней границе для кода и русского текста: оценка должна
завышать расход, а не занижать.
"""


def result_chars_for(context_window: int) -> int:
    """Предел результата инструмента, выведенный из окна модели (D-030)."""
    if context_window <= 0:
        return MAX_TOOL_RESULT_CHARS
    return max(MAX_TOOL_RESULT_CHARS, context_window * CHARS_PER_TOKEN // RESULTS_PER_WINDOW)
...
class NavigationToolbox:
    ...
    def fit_window(self, context_window: int) -> None:
        """Подгоняет предел результата под окно модели.

        Вызывается после первого ответа: какая модель ответит, решает роутер,
        и её окно известно только из ответа.
        """
