...
- **MCP-сервер** — навигация по индексу для внешних агентов: поиск по смыслу,
  определения, граф вызовов.
...
Одна команда поднимает API, оба воркера и фронт, сводит их логи в один поток
с пометкой источника и гасит всех по Ctrl+C. Процессы при этом остаются
раздельными — профили нагрузки у индексации и ревью разные (D-013).

```bash
uv run ducktective dev --reload        # перезапускать API по правкам
uv run ducktective dev --no-web        # без фронта
uv run ducktective dev --no-workers    # только API
```

По отдельности, если нужно смотреть за кем-то одним:
...
Ducktective разбирает кодовую базу и складывает из неё справочник: где какой
символ начинается и кончается, кто кого вызывает, какой фрагмент про что.
MCP-сервер открывает этот справочник наружу — Claude Code, Cursor и другие
клиенты могут в него заглядывать.

Пять инструментов:

| Инструмент | Отвечает на вопрос |
|---|---|
| `list_repositories` | какие базы проиндексированы и насколько свежи |
| `search_code` | где это сделано — по смыслу, а не по буквам |
| `get_definition` | как выглядит этот символ целиком |
| `find_callers` | кто его вызывает, то есть что сломается |
| `get_file_context` | что происходит на этих строках файла |

Сильная сторона — связи, на которых буксует `grep`: «кто вызывает», «где ещё
так делают», «что покрывает эти строки». Одноимённые символы возвращаются все,
соседи по графу — сигнатурой без тела.

Чего он не умеет, и это осознанно: **git-история** (ветки, коммиты, авторы —
у клиента уже есть `git log`), **рабочая копия** (ответы описывают
зафиксированную ревизию; смешав её с недописанным, индекс перестал бы быть
надёжным) и **прогоны ревью** — находки, severity и отметки лежат в других
таблицах, этот сервер их не читает.

Каждый ответ подписан ревизией индекса и датой сборки: по ним видно,
насколько он свеж. После коммитов индекс пересобирается — `ducktective index .`.
